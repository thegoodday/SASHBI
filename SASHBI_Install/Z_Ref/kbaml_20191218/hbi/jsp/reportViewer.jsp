...
%>
	...
	<script src="/<%=contextName%>/scripts/SlickGrid/lib/jquery-1.11.2.min.js"></script>
	<script src="/<%=contextName%>/scripts/SlickGrid/lib/jquery-ui-1.11.3.js"></script>
	<script src="/<%=contextName%>/scripts/jquery/js/jquery-migrate-1.2.0.js"></script>
<!--	
	<script src="/<%=contextName%>/scripts/jquery/js/jquery-1.9.1.js"></script>
	<script src="/<%=contextName%>/scripts/jquery/js/jquery-migrate-1.2.0.js"></script>
-->	
...
%>
	...
	<script src="/<%=contextName%>/scripts/SlickGrid/lib/jquery.event.drag-2.3.0.js"></script>
	<script src="/<%=contextName%>/scripts/SlickGrid/lib/jquery.event.drop-2.3.0.js"></script>
<!--	
	<script src="/<%=contextName%>/scripts/SlickGrid/lib/jquery.event.drag-2.2.js"></script>
	<script src="/<%=contextName%>/scripts/SlickGrid/lib/jquery.event.drop-2.2.js"></script>
-->		
...
for (int i = 0; i < promptDefinitions.size(); i++) {
	...
	if((pdi instanceof DateRangeDefinition) || (pdi instanceof SharedDateRangeDefinition)){
		...
		if (dateType.equals("DATE")) {
			...
			out.println("\t$(\"#slt" + pName + "_start\").bind('click',function(){$('.ui-datepicker-calendar').css('display','none');});");
			out.println("\t$(\"#slt" + pName + "_start\").bind('focus',function(){$('.ui-datepicker-calendar').css('display','none');});");
		} else if (dateType.equals("YEAR")) {
		...
		if (dateType.equals("DATE")) {
			...
			out.println("\t$(\"#slt" + pName + "_end\").bind('click',function(){$('.ui-datepicker-calendar').css('display','none');});");
			out.println("\t$(\"#slt" + pName + "_end\").bind('focus',function(){$('.ui-datepicker-calendar').css('display','none');});");
		} else if (dateType.equals("YEAR")) {
...
function checkSession(res){
	...
	console.log("create session save_path : " + save_path);
...
%>
			...
			//$("#_startDate1").val() = $("#slt<%=pdi.getPromptName()%>_start").val().replace(/-/gi, "");
			//$("#_startDate2").val() = $("#slt<%=pdi.getPromptName()%>_start").val().replace(/-/gi, "");
			//$("#_endDate1").val() = $("#slt<%=pdi.getPromptName()%>_end").val().replace(/-/gi, "");
			//$("#_endDate2").val() = $("#slt<%=pdi.getPromptName()%>_end").val().replace(/-/gi, "");
