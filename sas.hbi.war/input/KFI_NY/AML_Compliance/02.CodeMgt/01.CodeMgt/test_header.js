...
function YesNoSelectEditor(args){
	...
	this.loadValue = function (item) {
		...
		for (var ii in arrRiskScore){
			...
			console.log("arrRiskScore : " + arrRiskScore[ii].OFAC	+ " : " + data[args.item.id].OFAC);
			if (arrRiskScore[ii].OFAC	== data[args.item.id].OFAC ) {
				data[args.item.id].RISK_SCORE = arrRiskScore[ii].RISK_SCORE; 	
			}
...
function CreateAddlHeaderRow() {
	console.log("sgGrid.getPreHeaderPanel()");
	console.log(sgGrid.getPreHeaderPanel());
	...
	console.log("$preHeaderPanel");
	console.log($preHeaderPanel);
	
	var headerColumnWidthDiff = sgGrid.getHeaderColumnWidthDiff();
	console.log("headerColumnWidthDiff : " + headerColumnWidthDiff);
	var m, header, lastColumnGroup = '', widthTotal = 0;
	
	for (var i = 0; i < columns.length; i++) {
		m = columns[i];
		console.log(m.columnGroup);
