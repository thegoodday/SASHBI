...
function YesNoSelectEditor(args){
	...
	this.init = function () {
		var optItem="";
		YesNoList=[{val:"",text:""},{val:"O",text:"O"},{val:"X",text:"X"}]
	...
	this.loadValue = function (item) {
		...
		if (fnIsNull(defaultValue)) defaultValue='';
		//console.log("defaultValue : " + defaultValue);
		$select.val(defaultValue);
		
		var data=sgGrid.getData().getItems();
		if (fnIsNull(data[args.item.id].FATF)) data[args.item.id].FATF='';
		if (fnIsNull(data[args.item.id].OFAC)) data[args.item.id].OFAC='';
		...
		if(data[args.item.id].FATF == 'O' || data[args.item.id].OFAC == 'O') {
            ...
            if (data[args.item.id].BASEL	>= 7 ) {
                data[args.item.id].RISK_SCORE = 101;
            }else if(fnIsNull(data[args.item.id].BASEL)) {
	    ...
	    if (data[args.item.id].RISK_SCORE==101){
            data[args.item.id].RISK='VH';
        }else if(data[args.item.id].RISK_SCORE==100){
            data[args.item.id].RISK='H';
        }else if(data[args.item.id].RISK_SCORE==66){
            data[args.item.id].RISK='M';
        }else if(data[args.item.id].RISK_SCORE==33){
            data[args.item.id].RISK='L';
        }
...
function BaselTextEditor(args) {
    ...
    this.loadValue = function (item) {        
		...
		if(data[args.item.id].FATF == '' && data[args.item.id].OFAC == '') {
            if (data[args.item.id].BASEL	>= 7 ) {
                data[args.item.id].RISK_SCORE = 101;
            }else if(fnIsNull(data[args.item.id].BASEL)) {
                data[args.item.id].RISK_SCORE = 100;
...
function RiskSelector(args){
	var $select;
	var defaultValue;
	var scope = this;
	this.init = function () {
		var optItem="";
		OptList=[{val:"",text:""},{val:"VH",text:"VH"},{val:"H",text:"H"},{val:"M",text:"M"},{val:"L",text:"L"}]
		for (var ii in OptList){
			optItem+="<option value='" + OptList[ii].val + "'>" + OptList[ii].text + "</option>";
		}
		$select = $("<SELECT tabIndex='0' class='editor-yesno'>"+ optItem +"</SELECT>");			
		$select.appendTo(args.container);
		$select.focus();
	...
	this.loadValue = function (item) {
		var data=sgGrid.getData().getItems();
		console.log("defaultValue : " + defaultValue);
		console.log("item[args.column.field] : " + item[args.column.field]);
      ...
      if (defaultValue == "VH"){
			item.RISK_SCORE = '101'; 
			item.RISK = 'VH'; 
      } else if (defaultValue == "H"){
			item.RISK_SCORE = '100'; 
			item.RISK = 'H'; 
      } else if (defaultValue == "M"){
			item.RISK_SCORE = '66'; 
			item.RISK = 'M'; 
      } else if (defaultValue == "L"){
			item.RISK_SCORE = '33'; 
			item.RISK = 'L'; 
      }
	...
	this.serializeValue = function () {
		console.log("serializeValue $select.val() : " + $select.val());
		//return ($select.val() == "O");
		return $select.val() ;
	};
	this.applyValue = function (item, state) {
...
function setSlickGrid(data){
	...
	//Get Max Sequence
	$sasResHTML=execSTPA("SBIP://METASERVER/KFI_NY/AML Compliance/00.Environments/StoredProcess/0201_Country_getMaxSeq(StoredProcess)",'fnSetMaxSeq');
...
function fnSaveGrid1() {
    ...
    for (var idx=0;idx < data.length; idx++) {
        ...
        if(fnIsNull(data[idx].CODE_NAME)) {
            //console.log("idx "+idx+" CODE_NAME is null");
            isValid=false;
            invalidColumn="CODE_NAME";
            invalidIdx=idx;
            break;
        }        
    }    
...
function fnConfirmSaveGrid1(){
	...
	var keep_var=['CRR_SCORE_KEY','CODE','CODE_NAME','FATF','OFAC','BASEL','RISK','MANUAL_RISK','RISK_SCORE','CHANGE_CURRENT_IND'];
	    ...
	    if (row >= dataLen) {
	        return {
	            "columns":{
	               ...
	               ,"CODE_NAME":{editor:eval("Slick.Editors.Text"), formatter:eval("RiskCodeFommater")}
	               ,"FATF":{editor:eval("YesNoSelectEditor"), formatter:eval("Slick.Formatters.text")}
	               ,"OFAC":{editor:eval("YesNoSelectEditor"), formatter:eval("Slick.Formatters.text")}
	               ,"BASEL":{editor:eval("BaselTextEditor"), formatter:eval("Slick.Formatters.text")}
	               ,"RISK":{editor:eval("Slick.Editors.Dummy"), formatter:eval("RiskFommater")}
	            }
            };
        }
    };    
