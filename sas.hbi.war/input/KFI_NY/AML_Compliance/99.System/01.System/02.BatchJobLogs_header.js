...
function LogFommater(row, cell, value, columnDef, dataContext) {
	var data=sgGrid.getData().getItems();
	console.log("row risk : " + data[row].islog);
	var islog=data[row].islog;
	var text="";
	if( value==null) value="";
	if (islog == 1){
		text = "<span style='font-weight:normal;color:#ff0000'>" + value + "</span>";
	} else {
		text = "<span style='font-weight:normal;' >" + value + "</span>";
	}
	return text;
}
...
function setSlickGrid(data){
	...
	columns[3].formatter=eval("LogFommater");
	columns[4].formatter=eval("LogFommater");
	...
	sgGrid.onDblClick.subscribe(function(e, args) {
		...
		console.log(islog);
		if (islog != 1) {
			alertMsg("Not exist Logfile...");
			return;
		}
		var jobname=data[row].JOBNAME;
		var sttime=data[row].STTIME;
		console.log("jobname : " + jobname);
		fnShowLog(jobname,sttime);
	});	
...
function fnShowLog(jobname,sttime){
	console.log('jobname : ' + jobname);
	console.log('sttime : ' + sttime);
	fomSASLog._program.value='SBIP://METASERVER/KFI_NY/AML Compliance/00.Environments/StoredProcess/showBatchLogs(StoredProcess)';
	fomSASLog.path.value=jobname;
	fomSASLog.logfile.value=sttime;
	fomSASLog.submit();
}