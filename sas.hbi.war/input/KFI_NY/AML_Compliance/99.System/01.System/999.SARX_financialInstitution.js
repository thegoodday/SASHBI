...
function fnConfirmSave(){
	...
	$("input:checkbox[name=INSTITUTION_ROLE_CODE]:checked").each(function(){
...
function cbLoadInstInfo(data){
	...
	setTimeout('$("#INSTITUTION_TYPE_CODE_TEXT").val(instInfo.INSTITUTION_TYPE_CODE_TEXT);',1000*1);
	...
	$("#INSTITUTION_SF_TYPE_CODE_TEXT").val(instInfo.INSTITUTION_SF_TYPE_CODE_OTHER);					//54
	...
	for (var ii=0; ii < INSTITUTION_ROLE_CODE.length; ii++){
		...
		$("input:checkbox[name=INSTITUTION_ROLE_CODE][value='" + institution_role_code + "']").prop('checked',true);
	};
