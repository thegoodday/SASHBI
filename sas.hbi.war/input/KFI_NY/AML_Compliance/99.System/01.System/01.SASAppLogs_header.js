...
function setSlickGrid(data){
	...
	sgGrid.onDblClick.subscribe(function(e, args) {
		...
		var logfile=data[row].filename;
		console.log("Logfile : " + logfile);
		fnShowLog(path,logfile);
	});	
...
function fnShowLog(path,logfile){
	console.log('path : ' + path);
	console.log('logfile : ' + logfile);
	fomSASLog._program.value='SBIP://METASERVER/KFI_NY/AML Compliance/00.Environments/StoredProcess/showSASLogs(StoredProcess)';
	fomSASLog.path.value=path;
	fomSASLog.logfile.value=logfile;
	fomSASLog.submit();
}