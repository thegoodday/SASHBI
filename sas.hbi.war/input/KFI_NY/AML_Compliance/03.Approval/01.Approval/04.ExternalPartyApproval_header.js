...
function CustomSelectEditor(args){
	...
	this.init_rep = function () {
		var optItem="";
		YesNoList=[{val:"Y",text:"REP. Person"},{val:"N",text:"Not REP. Person"}]
		for (var ii in YesNoList){
			optItem+="<option value='" + YesNoList[ii].val + "'>" + YesNoList[ii].text + "</option>";
		}
		$select = $("<SELECT tabIndex='0' class='editor-yesno'>"+ optItem +"</SELECT>");			
...
function setSlickGridTree(sasResData) {
	...
	columns[4].editor=eval("CustomSelectEditor");	
	columns[4].formatter=eval("CustomSelectFommater");
...
function fnSaveGrid1() {
    ...
    for (var idx=0;idx < data.length; idx++) {
        ...
        if(parent_cluster > -1 && data[idx].indent == 1 && data[idx].cluster == parent_cluster) {
            if(data[idx].CHECK_YN=='Y') hasPEP = 1;
        }
        ...
        if(data[idx].indent == 1 && data[idx].CHECK_YN=='Y') {
            saveGridData.push(data[idx]);
        }
    }
...
function fnConfirmSaveGrid1(){
	...
	tParams['_program']="SBIP://METASERVER/KFI_NY/AML Compliance/00.Environments/StoredProcess/0301_ExternalPartyApproval(StoredProcess)";
	//console.log("gridData : ");
	//console.log(gridData);
	//console.log(tParams);
...
function downloadSarxEFile(){
    execTestSTP("SBIP://METASERVER/KFI_NY/AML Compliance/00.Environments/StoredProcess/downloadSarxEfile(StoredProcess)");
}
...
//    $("#dvToolBar").append("<input type=button value=Download id=btnSave class=condBtn onclick='downloadSarxEFile();'>");
