...
function setSlickGrid(data){
	...
	sgGrid.onDblClick.subscribe(function(e, args) {
		...
		$sasResHTML=execSTPA("SBIP://METASERVER/KFI_NY/AML Compliance/00.Environments/StoredProcess/0302_getMacthResult(StoredProcess)",'setMacthResultGrid');
...
function dispCompliance(data){
	...
	if(parseInt(data[0].approvalkey) >= 30){
		...
		$("#txtLvl2Detail").val(data[0].L2_RESULT_DETAIL);
  ...
  if(parseInt(data[0].approvalkey) >= 40){
		...
		$("#txtLvl4Detail").attr("readonly", true);
		$("#sltLvl4Trst").prop("disabled", true);
	}

}

function resetComplianceInput(){
...
*/
	...
	for(ii in sgData){
		var objTemp = $.extend(sgData[ii],eval({"id":ii}));
		if(sgData[ii].POLITICALLY_EXPOSED_PERSON_IND == 'Y'){
			pepCnt++;
		}
	}
