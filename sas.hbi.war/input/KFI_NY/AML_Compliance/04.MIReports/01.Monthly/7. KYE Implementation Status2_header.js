$(document).ready(function () {
	...
	$("#dvToolBar").append("<input type=button value='Load Screenning Result' id=btnSave class=condBtn onclick='fnLoadResult();'>");
...
function setSlickGrid(data){
	...
	var P_BASE_DATE_start=$("#sltP_BASE_DATE_start").val();
	var P_BASE_DATE_end=$("#sltP_BASE_DATE_end").val();
	console.log("P_BASE_DATE_start : " + P_BASE_DATE_start);
	console.log("P_BASE_DATE_end : " + P_BASE_DATE_end);
	if (	P_BASE_DATE_start == P_BASE_DATE_end) {
		$("#dvToolBar").show();
	} else {
		$("#dvToolBar").hide();
	}
	...
	columns[6].editor=eval("ApprEditor");	
...
*/	
	...
	tParams['target']=$("#sltTarget").val();;
	tParams['_library']="stp_temp";
	tParams['_tablename']="fsb_emp_know_result";
	tParams['_program']="SBIP://METASERVER/KFI_NY/AML Compliance/00.Environments/StoredProcess/0703_saveAppropriateStatus(StoredProcess)";
	console.log("gridData : ");
	console.log(gridData);
	console.log(tParams);
...
function fnLoadResult(){
	//console.log(sgData);
	tParams=eval('[{}]');
	$sasResHTML=execSTPA("SBIP://METASERVER/KFI_NY/AML Compliance/00.Environments/StoredProcess/0704_loadResult(StoredProcess)",'setSlickGrid');
}