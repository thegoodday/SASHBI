...
function resizeFrame(){
	...
	$("#dvCol1").width(350);														
		...
		$("#dvMeasure").width($("#dvCol1").width());													
	$("#dvCol2").width(eval(($(window).width()-$("#dvCol1").width())*0.45));														
	$("#dvCol3").width(eval($(window).width()-$("#dvCol1").width()-$("#dvCol2").width()-32));														
															
	$("#dvColDFT").height(eval($(window).height()*0.35));														
	$("#dvColUser").height(eval($(window).height()*0.1));														
	$("#dvMeasure").height(eval($(window).height()-$("#dvCondi").height()-$("#dvColDFT").height()-$("#dvColUser").height()-$("#dvStat").height()-175));														
	if($("#dvMeasure").height() == 70){														
		$("#dvMain").css("overflow-y","auto");													
	}														
															
	$("#treeLayout").height(eval($(window).height()-$("#dvCondi").height()-$("#treeMStats").height()-71));														
															
	$("#dvColDFT").height(eval($(window).height()*0.4));														
															
	$("#treeFilter").height(eval($(window).height()-$("#dvCondi").height()-$("#dvFilterHeader").height()-70));														
}
var	curColumnName;
...
$(document).ready(function () { 
	...
	$(".buttonArea").width(350);
	$(".buttonArea").css("padding-top","5px");
	$("#sltLIBRARY").css("width","160px");
	$("#sltTREE_TYPE").css("width","160px");
	$("#sltDATA_NAME").css("width","447px");
...
var isFirst=0;
function openLayout(){
	var orgsltRPT_GRP=$("#sltRPT_GRP").val();
	$("#dvBG").css("height",$(window).height()+"px");
	$("#dvBG").width($(window).width());
	$('#dvBG').hide();
...
function openTreeLayout(){
  ...
  LIBNAME=dataRptList[current_row].LIBNAME;
  DATA_NAME=dataRptList[current_row].DATA_NAME;
  ...
  treeDesc=dataRptList[current_row].TREE_DESC;
	...
	onChange_ACCT_GROUP();
	setTimeout("setTREE_TYPE('"+treeID+"','"+treeName+"','"+TREE_TYPE+"','"+SUB_TYPE+"','"+LIBNAME+"','"+DATA_NAME+"');",1000*1);
	//execAjax("openTreeRollUp","",true,"openTreeLayout2","json");
}
function setTREE_TYPE(treeID,treeName,TREE_TYPE,SUB_TYPE,LIBNAME,DATA_NAME){
	//console.log("setTREE_TYPE called...");
	$("#sltTREE_TYPE").val(TREE_TYPE).attr("selected", "selected");	
	console.log("TREE_TYPE : " + TREE_TYPE);
	onChange_TREE_TYPE();
	setTimeout("setSUB_TYPE('"+treeID+"','"+treeName+"','"+SUB_TYPE+"','"+LIBNAME+"','"+DATA_NAME+"');",1000*1);
}
function setSUB_TYPE(treeID,treeName,SUB_TYPE,LIBNAME,DATA_NAME){
	$("#sltSUB_TYPE").val(SUB_TYPE).attr("selected", "selected");	
	onChange_SUB_TYPE();
	setTimeout("setLIBNAME('"+treeID+"','"+treeName+"','"+LIBNAME+"','"+DATA_NAME+"');",1000*1);
}
function setLIBNAME(treeID,treeName,LIBNAME,DATA_NAME){
	console.log("LIBNAME : " + LIBNAME);
	$("#sltLIBNAME").val(LIBNAME).attr("selected", "selected");	
	onChange_LIBRARY();
	setTimeout("setDATA_NAME('"+treeID+"','"+treeName+"','"+DATA_NAME+"');",1000*1);
}
function setDATA_NAME(treeID,treeName,DATA_NAME){
	$("#sltDATA_NAME").val(DATA_NAME).attr("selected", "selected");	
	get_Columns();
	console.log("DATA_NAME : " + DATA_NAME);
	tParams=eval('[{}]');
	tParams['TREE_ID']=treeID;
...
function saveTreeLayout(){
	...
	console.log(JSON.stringify(objFlatMStats));
	console.log(objFlatMStats);
	
	var RPT_TYPE=$("#sltRPT_TYPE option:selected").val();
	var ORG_GROUP="";
	var TREE_TYPE=$("#sltTREE_TYPE option:selected").val();
	var LIBRARY=$("#sltLIBRARY option:selected").val();
