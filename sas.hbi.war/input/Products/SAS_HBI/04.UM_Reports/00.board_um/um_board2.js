...
$(document).ready(function () {
    $("#dvToolBar").append("<input type=button value='글 등록' id=btnAdd class=condBtn onclick='fnRegistArticle();'>");
    $("#dvToolBar").append("<input type=button value='글 삭제' id=btnDelete class=condBtn onclick='fnConfirmDelete();'>");
...
function cbConfirmRegistArticle() {
    ...
    updata.append("BOARD_TITLE", artTitle2);
    updata.append("BOARD_CONTENT", artContent2);
    updata.append("USER_NAME", userID);
    updata.append(
        "_program",
        "SBIP://METASERVER/Products/SAS HBI/00.Environments/StoredProcess/02.STP Reports/02.Board_um/01.registArticle_um(StoredProcess)"
    );
...
function fnRegistArticle() {
    ...
    $("#tdTitle").html('<input type=text id=iptTitle size=20 style="width:200px;" maxlength="100">');
    $("#iptTitle").val("");
...
function updateViewCnt() {
    ...
    execAjax(
        "",
        "SBIP://METASERVER/Products/SAS HBI/00.Environments/StoredProcess/02.STP Reports/02.Board_um/01.updateViewCnt(StoredProcess)",
        true,
        "cbUpdateViewCnt",
        "json",
        debug
...
function getAttachFiles(rowInfo) {
    ...
    execAjax(
        "",
        "SBIP://METASERVER/Products/SAS HBI/00.Environments/StoredProcess/02.STP Reports/02.Board_um/01.getAttachFiles(StoredProcess)",
        true,
        "cbGetAttachFiles",
        "json",
        debug
...
function fnDownloadFile(row) {
    ...
    var url =
        ...
        "SBIP://METASERVER/Products/SAS HBI/00.Environments/StoredProcess/02.STP Reports/02.Board_um/01.downloadFile(StoredProcess)";
...
function cbConfirmUpdateArticle() {
    ...
    var artTitle = encodeURIComponent(curRow.BOARD_TITLE);
    var artContent = editor.getValue();
    artContent2 = encodeURIComponent(artContent);
    update.append("BOARD_TITLE", artTitle);
    update.append("BOARD_CONTENT", artContent2);
    update.append("USER_NAME", userID);

    sgData = sgGridAttachFiles.getData().getItems();
    update.append("ATTACH_INFO", JSON.stringify(sgData));
    update.append(
        "_program",
        "SBIP://METASERVER/Products/SAS HBI/00.Environments/StoredProcess/02.STP Reports/02.Board_um/01.updateArticle(StoredProcess)"
    );
...
function fnExecuteCode() {
    ...
    update.append(
        "_program",
        "SBIP://METASERVER/Products/SAS HBI/00.Environments/StoredProcess/02.STP Reports/02.Board_um/01.executeCode(StoredProcess)"
    );
    ...
    $.ajax({
        ...
        type: "post",
        url: "/SASStoredProcess/do",
        _program: "SBIP://METASERVER/Products/SAS HBI/00.Environments/StoredProcess/02.STP Reports/02.Board_um/01.executeCode(StoredProcess)",
        dataType: "json",
        data: update,
        processData: false,
        contentType: false,
...
function cbConfirmDelete() {
    var selectedIndexes = sgGrid.getSelectedRows().sort().reverse();
    console.log("selectedIndexes : " + selectedIndexes);
    var sgData = sasGrid.getData().getItems();
    var ARTICLE_IDs = "";
    for (ii in selectedIndexes) {
        tParams = eval("[{}]");
    ...
    execAjax(
        "",
        "SBIP://METASERVER/Products/SAS HBI/00.Environments/StoredProcess/02.STP Reports/02.Board_um/02.deleteArticle_um(StoredProcess)",
        true,
        "cbDeleteArticle",
        "json",
        debug
