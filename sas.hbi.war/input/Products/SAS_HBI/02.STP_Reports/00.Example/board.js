...
function setSlickGrid(data) {
    ...
    var TitleFormatter = function (row, cell, value, columnDef, dataContext) {
        ...
        var spacer = "<span style='display:inline-block;height:1px;width:" + (15 * eval(dataContext["level"] - 0)) + "px'></span>";
        var idx = dataView.getIdxById(dataContext.id);
        if (sgData[idx + 1] && sgData[idx + 1].level > sgData[idx].level) {
    ...
    columns[2].formatter = TitleFormatter;
    ...
    for (ii in sgData) {
        ...
        var objTemp = $.extend(sgData[ii], eval({ "id": sgData[ii].article_id }));
    }
    ...
    function articleFilter(item) {
        if (item.parent_id != 0) {
            var parent = sgData.filter(row => row.article_id == item.parent_id)[0];
            while (parent) {
                if (parent._collapsed) {
                    return false;
                } 
                parent = sgData.filter(row => row.article_id == parent.parent_id)[0];
            }
        }
        return true;
    }
    ...
    dataView.setItems(sgData);
    dataView.setFilter(articleFilter);
    dataView.endUpdate();
    $("#sasGrid").height(eval($(window).height() - $("#dvCondi").height() - 75));
...
function cbConfirmRegistArticle() {
    ...
    updata.append(
        "_program",
        "SBIP://METASERVER/Products/SAS HBI/00.Environments/StoredProcess/02.STP Reports/00.Board/01.registArticle(StoredProcess)"
    );
...
function getAttachFiles(rowInfo) {
    ...
    execAjax(
        "",
        "SBIP://METASERVER/Products/SAS HBI/00.Environments/StoredProcess/02.STP Reports/00.Board/01.getAttachFiles(StoredProcess)",
        true,
        "cbGetAttachFiles",
        "json",
        debug
...
function fnDownloadFile(row) {
    ...
    var url =
        ...
        "SBIP://METASERVER/Products/SAS HBI/00.Environments/StoredProcess/02.STP Reports/00.Board/01.downloadFile(StoredProcess)";
...
function fnConfirmThread() {
    ...
    isHideBG = false;
    confirmMsg("Are you sure you want to save?", "cbConfirmThread");
}
...
function cbConfirmUpdateArticle() {
    ...
    update.append(
        "_program",
        "SBIP://METASERVER/Products/SAS HBI/00.Environments/StoredProcess/02.STP Reports/00.Board/01.updateArticle(StoredProcess)"
    );
...
function cbConfirmDelete() {
    ...
    execAjax(
        "",
        "SBIP://METASERVER/Products/SAS HBI/00.Environments/StoredProcess/02.STP Reports/00.Board/01.deleteArticle(StoredProcess)",
        true,
        "cbDeleteArticle",
        "json",
        debug
