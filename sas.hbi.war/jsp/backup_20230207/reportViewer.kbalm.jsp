...
logger.info("lastAccTime : " + lastAccTime);
String desc="";
String dftVal="";
...
for (int ii=1;ii<groupList.size();ii++){
	String grpName = (String)groupList.get(ii);
	logger.debug("Group Name : " + grpName);
	if ( grpName.startsWith("ALM")){
		almGroup.add(grpName);	
	}
}
String almGroupS = almGroup.toString();
logger.info("almGroup : " + almGroupS);
...
function execAjax(url,sp_URI,isAsync,fn,dataType,param1,param2,param3,param4,param5,param6,param7,param8,param9,param10,param11,param12,param13,param14,param15){
	...
	try {
		...
		console.log("[ERROR] tParams length property can not read... ");
	}
	console.log("param");
	console.log(param);
...
function getParamVal(sp_URI,tableName,colName,target,libStmt,param1,param2,param3,param4,param5,param6,param7,param8){
	console.log("tParams in getParamVal");
	console.log(tParams);
	$("#progressIndicatorWIP").css("left",eval(eval($(window).width()-$("#progressIndicatorWIP").width()-300)/2));
	$("#progressIndicatorWIP").css("top",eval(eval($(window).height()-$("#progressIndicatorWIP").height()-300)/2));
	updateTimeoutHBI();
	...
	*/
	...
	for (int i = 0; i < promptDefinitions.size(); i++) {
		...
		if (isTextBox=="Y") {
			...
			hStr="\tvar " + pdi.getPromptName() + "=$(\"#slt" + pdi.getPromptName()+ " option:selected\").val();";
			out.println(hStr);
			hStr="console.log('" + pdi.getPromptName() +" in getParamVal() :'+" + pdi.getPromptName() +");";
			out.println(hStr);
		}
		paramInfo.put(pdi.getPromptName().toUpperCase(), pdi.getPromptName());
		//out.println(hStr);
...
<div id=dvCondi>
	<table width="100%" >
	    <tr style="padding:0px;">
	        <td width=10 style="padding:0px;"><!--img src="/<%=contextName%>/images/ico_titleType2.png" border="0"  style="padding-top:5px;"--></td>
	        <th class="t ReportTitle" style="padding:0px;"><%=desc%></th>
           <td align="right" v-align="bottom" style="padding:0px;">
...
for (int i = 0; i < promptDefinitions.size(); i++) {
	...
	if (pdi instanceof TextDefinition){
		...
		hStr="<span class='titleimg condItem' " + isPromptHidden + "><table id=cdTbl" + pdi.getPromptName() + " class=itemTable><tr><td id=cdLabel" + pdi.getPromptName() + " class='condLabel'>"+pdi.getPromptLabel(locale)
				...
				+ "' style='width:72px;text-align:center;' size=8 value='' ><img src='/" + contextName + "/images/Calendar.gif' border=0 onClick='slt"+pdi.getPromptName()+"_DD.focus();' style='cursor:pointer'></td></tr></table></span>";
		out.println(hStr);
	} else if (pdi instanceof DateRangeDefinition){
		hStr="<span class='titleimg condItem' " + isPromptHidden + "><table id=cdTbl" + pdi.getPromptName() + " class=itemTable><tr><td id=cdLabel" + pdi.getPromptName() + " class='condLabel'>"+pdi.getPromptLabel(locale)
...
%>
		...
		-->
		<img id="btnHelp"  src="/<%=contextName%>/images/icon_help.png" border=0 onclick="getSASLog();" >
...
%>
	...
	if (<%=pdi.getPromptName()%>_start != undefined && <%=pdi.getPromptName()%>_start != "") $("#slt<%=pdi.getPromptName()%>_start").val(<%=pdi.getPromptName()%>_start);
	var <%=pdi.getPromptName()%>_end = getCookie('<%=pdi.getPromptName()%>_end');
	if (<%=pdi.getPromptName()%>_end != undefined && <%=pdi.getPromptName()%>_end != "") $("#slt<%=pdi.getPromptName()%>_end").val(<%=pdi.getPromptName()%>_end);
	console.debug("<%=pdi.getPromptName()%>_start : " + <%=pdi.getPromptName()%>_start);
	console.debug("<%=pdi.getPromptName()%>_end : " + <%=pdi.getPromptName()%>_end);
<%			
		} else if ((pdi instanceof DateDefinition) || (pdi instanceof SharedDateDefinition)) {
%>
	var <%=pdi.getPromptName()%>_DD = getCookie('<%=pdi.getPromptName()%>_DD');
	if (<%=pdi.getPromptName()%>_DD != undefined && <%=pdi.getPromptName()%>_DD != "") $("#slt<%=pdi.getPromptName()%>_DD").val(<%=pdi.getPromptName()%>_DD);
	console.debug("<%=pdi.getPromptName()%>_DD : " + <%=pdi.getPromptName()%>_DD);
<%			
		} 
	}
%>
