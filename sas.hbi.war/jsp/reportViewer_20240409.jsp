...
boolean inNoInit = false;
if(hiddenInfo.containsKey("_STPREPORT_NO_INIT")){
	if ("1".equals(hiddenInfo.get("_STPREPORT_NO_INIT"))) inNoInit = true;
}
boolean isChart = false;
if(hiddenInfo.containsKey("_STPREPORT_IS_CHART")){
	if ("1".equals(hiddenInfo.get("_STPREPORT_IS_CHART"))) isChart = true;
}
...
%>
	...
	<script src="/SASHBI/scripts/jquery/reportViewer.js"></script>
<%
...
if (rptType.equalsIgnoreCase("tabHTML")){
%>	
	<script src="/SASHBI/scripts/jquery/tabHTML.js" charset="euc-kr"></script>
<%
} else if (rptType.equalsIgnoreCase("tabGrid")){
%>	
	<script src="/SASHBI/scripts/jquery/tabGrid.js" charset="euc-kr"></script>
<%
} else if (rptType.equalsIgnoreCase("opnSWSlickGrid")){
%>	
	<script src="/SASHBI/scripts/jquery/grid.js" charset="euc-kr"></script>
<%
} else if (rptType.equalsIgnoreCase("opnSWSlickGridEdit")){
%>	
	<script src="/SASHBI/scripts/jquery/gridEditor.js" charset="euc-kr"></script>
<%
} 
%>
<%
if (headerJS != null){
%>	
	<script src="<%=headerJSURL%>" charset="euc-kr" type="text/html"></script>
<%
}
%>
<%
...
function dowloadExcel(){
	...
	myForm.save_path.value = save_path;
	myForm._reportName.value = _REPORT_DESC;
	myForm.param1.value = "ExcelDown";
	myForm.sas_forwardLocation.value = "downFile";
<%
	for (int i = 0; i < promptDefinitions.size(); i++) {
		...
		if (pdi instanceof DateRangeDefinition || pdi instanceof SharedDateRangeDefinition) {
			...
			if (!pName.startsWith("_STP")){
				hStr="\tmyForm." + pdi.getPromptName() + ".value" + "=$(\"#slt" + pdi.getPromptName()+ "\").val();";
				out.println(hStr);
				hStr="\tmyForm." + pdi.getPromptName() + "_txt.value" + "=$(\"#slt" + pdi.getPromptName()+ " option:selected\").val();";
				out.println(hStr);
			}
		}
	}
%>
	$("select").each(function(){
		...
		if (name != undefined && name.length > 3){
			...
			let isExist = document.getElementByName(name);
			//
			if (isExist.length == 0){
				let newParam = document.createElement("input");
				newParam.setAttribute("name", name);
				newParam.setAttribute("value", $(this).val());
...
%>
			...
			<td class="buttonArea" id="condButtonArea">	
				<div id="viewableDateMsg" class="viewableLabel" style="width:240px height:20px;position: absolute; top12px; right:80px"></div>	
<%
	if (isNoSubmitBtn == 0){
%>			
...
<div id=dvSubImg class="none_data" style="displa:block">
